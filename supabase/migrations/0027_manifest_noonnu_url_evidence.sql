-- 목적(#150): 눈누 상세 본문 앵커를 근거로 official_url / license_source_url을 정정할 수 있게 한다.
-- 0026이 official_url을 허용 필드 목록에는 넣었지만 근거 검증 우회 목록에는 넣지 않아
-- confidence('reference') <> source_kind('noonnu') 규칙에서 거부됐다(dev 실측).
-- license_source_url은 license_% 규칙에 걸려 document_kind='license' + official/public을 요구했다.
-- 0026 원문 그대로 복사한 뒤 근거 검증 우회 블록만 확장한다.
...
begin
  ...
  for v_entry in select value from jsonb_array_elements(v_manifest->'entries') loop
    ...
    for v_finding in select f from jsonb_array_elements(v_manifest#>'{evidence_bundle,findings}') f
      ...
      -- 이슈 #150: 눈누 official_url 오염 정정. 눈누 상세 본문 앵커 근거로
      -- official_url/license_source_url을 고치려면 이 우회가 필요하다. 전용 마커
      -- noonnu-official-url-anchor로 한정해 font-file-script(폰트 파일 실검사)와
      -- 근거 등급이 섞이지 않게 한다.
      if v_key in ('foundry','foundry_url','download_url','download_source_kind','license_source_url','official_url')
         and v_snapshot->>'document_kind'='metadata'
         and (
           (v_snapshot->>'source_kind'='noonnu' and v_snapshot#>>'{extracted,evidence_role}' in ('font-file-script','noonnu-official-url-anchor'))
