-- 목적: dev manifest apply RPC가 'finding UUID content conflict'로 291/291 전량 실패하는
-- 문제를 고친다.
--
-- 원인: 파이프라인 writer(apps/pipeline/src/fontagit_pipeline/audit_store.py)는
-- before_value/proposed_value(그리고 스냅샷 extracted/evidence_locations)가 없을 때
-- SQL NULL로 저장한다. 그런데 이 함수의 findings/snapshots 재적용 충돌 검사는
-- manifest의 jsonb 'null'과 `is distinct from`으로 원시 비교해 `null::jsonb is distinct
-- from 'null'::jsonb`가 true로 평가된다. 같은 함수의 entries 재적용 검사는
-- `fontagit._audit_font_value`(0018)가 `coalesce(...,'null'::jsonb)`로 정규화해 이 문제를
-- 피하는데, findings/snapshots 루프만 이 정규화가 누락돼 있었다.
--
-- 변경 (0024 원문을 복사해 아래 4곳만 수정, 나머지 로직은 동일):
-- 1) findings 루프 충돌 검사: before_value/proposed_value 비교를 양쪽
--    coalesce(...,'null'::jsonb) 정규화로 교체.
-- 2) snapshots 루프 충돌 검사: extracted/evidence_locations 비교도 동일하게 정규화.
-- 3) 두 충돌 예외 메시지에 식별자(finding id/field_name, snapshot id) 포함.
-- 4) 저장 표현 단일화: findings insert의 before_value/proposed_value, snapshots insert의
--    extracted/evidence_locations 값을 nullif(...,'null'::jsonb)로 감싸 SQL NULL로 통일.
