-- 이슈 #133 (PR #133 리뷰): Tier A(google-fonts) 근거 스냅샷은 raw.githubusercontent.com
-- 원문이며 source_registry.json 기준 archive 등급이다. 그런데 font_source_snapshots.source_kind
-- CHECK(0017:77)가 official/public/noonnu만 허용해 어쩔 수 없이 'public'으로 저장했고,
-- 이 값이 일반 metadata 승인 분기(apply_font_audit_manifest, evidence_role_is_valid)의
-- official/public 허용 범위와 겹쳐 5필드 우회 밖에서도 name_en/tags/weights/variants 등이
-- "공공기관 등급 근거"로 잘못 승인될 수 있었다. archive를 저장 등급으로 추가해 등급 체계
-- 모순을 없앤다.
--
-- font_source_snapshots.source_kind는 0017 create table 내부의 이름 없는 인라인 check라
-- Postgres가 자동 생성한 제약 이름에 의존하면 안전하지 않다. pg_constraint에서 실제
-- 제약 이름을 조회해 drop한다(0021처럼 이름을 하드코딩하지 않음).
--
-- apply_font_audit_manifest(0023)의 5필드 우회 조건도 archive를 포함하도록 맞춘다.
-- provider=google-fonts와 evidence_role=tier-a-metadata-pb 마커로 판정하므로 archive여도
-- 동일하게 동작해야 한다(과거 데이터 호환을 위해 public도 계속 허용). 0022가 0021의
-- download_source_kind 확장에 맞춰 값 검증자를 갱신한 것과 같은 패턴이다.
...
begin
  ...
  where ns.nspname = 'fontagit'
    and rel.relname = 'font_source_snapshots'
    and con.contype = 'c'
    and att.attname = 'source_kind';

  if v_constraint_name is not null then
    execute format(
      'alter table fontagit.font_source_snapshots drop constraint %I',
      v_constraint_name
    );
  end if;
end;
...
alter table fontagit.font_source_snapshots
  add constraint font_source_snapshots_source_kind_check
    check (source_kind in ('official', 'public', 'noonnu', 'archive'));
