...
-- 링크·표기 등급이며 legal 필드(allow_*)나 license_source_kind와 달리 사람 검수 전
-- reference 신뢰도 근거(눈누 font-file-script metadata, Tier A google/fonts METADATA.pb)를
-- 허용한다. 0019의 apply_font_audit_manifest 원문을 그대로 복사하고 evidence 검사 블록
-- 맨 앞에 5개 필드용 우회(continue)만 추가한다. legal 필드/license_source_kind는 미변경.
    ...
    for v_finding in select f from jsonb_array_elements(v_manifest#>'{evidence_bundle,findings}') f
      ...
      if v_key in ('foundry','foundry_url','download_url','download_source_kind','license_source_url')
         ...
         and (
           ...
           or (v_snapshot->>'source_kind'='public' and v_snapshot->>'provider'='google-fonts'
