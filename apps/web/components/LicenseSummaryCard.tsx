...
const LICENSE_HELP = {
  commercial: `광고${String.fromCharCode(183)}상품${String.fromCharCode(183)}웹사이트${String.fromCharCode(183)}영상 등 상업 활동의 결과물에 폰트를 사용할 수 있는지 뜻해요.`,
  modify: "글자 모양이나 폰트 파일을 바꿔 파생 폰트를 만들 수 있는지 뜻해요.",
  redistribute: "폰트 파일 자체를 다른 사람에게 다시 제공할 수 있는지 뜻해요.",
  embedding: `폰트 파일을 PDF${String.fromCharCode(183)}전자책${String.fromCharCode(183)}앱 같은 문서나 프로그램 안에 포함할 수 있는지 뜻해요.`,
  fontSale: "폰트 파일 자체를 단독 상품으로 판매할 수 있는지 뜻해요. 폰트를 사용한 결과물 판매와는 달라요.",
  attribution: `폰트 사용이나 파일 배포 시 제작자${String.fromCharCode(183)}라이선스 정보를 밝혀야 하는지 확인하는 항목이에요.`,
  ...
  return (
    <aside className={styles.card}>
      ...
      <div className={styles.links}>
        ...
        {!isLegacy && audit?.sourceUrl && <a href={audit.sourceUrl} target="_blank" rel="noopener noreferrer">{sourceLabel} {String.fromCharCode(183)} 원문 보기</a>}
      </div>
    </aside>
  );
}
