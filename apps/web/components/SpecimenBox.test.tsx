...
describe("SpecimenBox", () => {
  it("기본 견본 문구를 렌더한다", () => {
    render(<SpecimenBox fontFamily="var(--font-x)" font={mockFont} editable={false} />);
    expect(screen.getByText("다람쥐 헌 쳇바퀴에 타고파")).toBeInTheDocument();
  });
  it("editable이면 입력이 견본을 갱신한다", () => {
    render(<SpecimenBox fontFamily="var(--font-x)" font={mockFont} editable initialText="가나다" />);
  ...
  it("caption을 렌더한다", () => {
    render(<SpecimenBox fontFamily="var(--font-x)" font={mockFont} editable={false} caption="대체 견본입니다" />);
