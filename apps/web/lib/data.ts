...
export function checkIntegrity(fontList: Font[], collectionList: Collection[], validKeys: FontKey[]): void {
  ...
  for (const c of collectionList) {
    ...
    for (const it of c.items) {
      if (itemSlugs.has(it.slug)) throw new Error(`컬렉션 내 중복 fontSlug: ${c.slug} -> ${it.slug}`);
      itemSlugs.add(it.slug);
      if (!fontList.find((font) => font.slug === it.slug)) throw new Error(`컬렉션 폰트 참조 오류: ${c.slug} -> ${it.slug}`);
    }
  }
}
