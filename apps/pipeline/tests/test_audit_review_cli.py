...
def test_audit_review_success_log_all_approved(caplog):  # type: ignore[no-untyped-def]
    ...
    with caplog.at_level(logging.INFO):
        with patch("fontagit_pipeline.audit_store.SupabaseAuditStore.from_dev_credentials") as mock_store_ctor:
            ...
            assert result == 0
            # 성공 로그 확인: "승인 완료: approved=2 failed=0 total=2"
            success_logs = [rec for rec in caplog.records if "승인 완료" in rec.message]
            assert len(success_logs) > 0, "성공 로그('승인 완료')가 출력되어야 합니다"
            assert "approved=2" in success_logs[0].message
            assert "failed=0" in success_logs[0].message
            assert "total=2" in success_logs[0].message


def test_audit_review_success_log_no_findings(caplog):  # type: ignore[no-untyped-def]
    """정상: 승인 대상이 없을 때도 성공 로그 출력 검증."""
    run = _mock_run()
    args = argparse.Namespace(
        action="auto-approve",
        run_id=run["id"],
    ...
    with caplog.at_level(logging.INFO):
        with patch("fontagit_pipeline.audit_store.SupabaseAuditStore.from_dev_credentials") as mock_store_ctor:
            mock_store = MagicMock()
            mock_store.get_run.return_value = run
            mock_store.get_proposed_findings.return_value = []
            mock_store_ctor.return_value = mock_store

            result = main_audit_review(args)

            assert result == 0
            # 성공 로그 확인: "승인 완료: approved=0 failed=0 total=0"
            success_logs = [rec for rec in caplog.records if "승인 완료" in rec.message]
            assert len(success_logs) > 0, "성공 로그('승인 완료')가 출력되어야 합니다"
            assert "approved=0" in success_logs[0].message
            assert "total=0" in success_logs[0].message
