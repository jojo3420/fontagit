...
- 상태: 완료 (모든 작업 브랜치 develop→main 통합, 워크트리/브랜치 정리, fontagit.com 라이브)
- 근본 원인: `apps/web/lib/db/fonts.ts`의 `getAllSlugs`/`getPublishedSlugs`가 `.range()` 페이지네이션 없이 조회 → PostgREST 기본 1000행 무음 절단. 전체 1,240종(한글 slug 1,062) 중 slug 정렬 1000행 이후(대부분 한글)가 `generateStaticParams`에서 누락 → 정적 페이지 미생성(라이브 404)-sitemap 누락. 같은 파일 `getAllFonts`의 기존 range 루프 패턴을 두 함수에 동일 적용해 해결(`.order("slug").range()`).
- 통합: `codex/search-index-task4a`(=`feature/noonnu-tier-b-enrich`, 417c15d가 상위집합 — 동일 slug fix + `sitemap.ts` encodeURIComponent 한글 인코딩)와 `codex/fix-tier-a-font-preview`(리뷰 문서 고유)를 develop에 머지. 충돌은 문서 2건만 수동 해소, fonts 파일은 canonical(task4a) 유지 → develop→main 머지.
- 검증: apps/web 테스트 180 passed, prod 빌드 정적 페이지 2,502개 전량 생성(기존 missing-param 에러 소멸), 라이브 `fontagit.com`에서 뒤쪽 정렬 한글 slug(yoon-흥수/yoon-감자꽃) HTTP 200, sitemap fonts URL 1,240 전량 확인.
- 커밋: slug fix `7a490ee`, develop 머지 `df11ff2`/`ca68b52`, develop→main 머지 `7a17c74`. Cloudflare Pages 배포 `f2a4e5d5.fontagit.pages.dev`, 스모크 200.
- 정리: 워크트리 3개(sitemap-finalize/sitemap-merge/verify-develop) 제거, 로컬/원격 codex-feature 브랜치 삭제(로컬은 develop/main만 잔존).
