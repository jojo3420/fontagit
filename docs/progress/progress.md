...
- 눈누 Tier B 라이선스-스타일 반자동 수집 파이프라인: 눈누 상세페이지에서 사실만 추출 → 명백히 안전한 것 자동 발행/애매하면 사람 검수. 마이그레이션 0016 + CLI 3종. **prod(ollidam)에 Tier B 1,110종 적재 완료(전체 1,240 published 검증)**. 리얼서버 배포는 보류중.

## 진행 기록

## 2026-07-18 - 눈누 Tier B 전량 수집 → prod 데이터 적재 완료
- 상태: 부분 완료 (prod 데이터 적재 완료-검증. deploy.sh 리얼서버 배포는 사용자 요청으로 보류)
- 요약: 눈누 Tier B 1,157종 전량 enrich 실행 → 자동발행 1,092 / 검수대기 44 / 스킵 21(이미 발행분 멱등 스킵). prod(ollidam) upsert 적재 후 실조회로 Tier B 1,110 / 전체 1,240 published 검증. sitemap 한글 slug 인코딩 + getAllSlugs/publish 조회 1000행 페이지네이션 fix(무음 절단 방지, auto발행 1,092>1000이라 필수였음). 자동 미발행 사유는 전부 license_proposals에 기록: 34건=상업 게이트 조건부, 10건=게이트 카테고리(포장지/영상) 빈칸.
- 커밋/PR: sitemap fix `f407a77`, publish 페이지네이션 fix(origin/develop 반영), PR #67 develop→main 머지 `2e5fdb3`. pipeline 테스트 147 passed.
- 남은 일: (1) deploy.sh 리얼서버 배포(prod 데이터 반영 — 보류중, 배포 시 main 콘텐츠+env 갖춘 워크트리 필요) (2) 검수대기 44건 noonnu-review 사람 검수 (3) 상세페이지 라이선스 4행 렌더 UI 후속
