...
- OFL 라이선스 자동 verified: OFL(오픈폰트라이선스) 폰트 132종을 google/fonts 공식 저장소로 확인해 라이선스 권한(상업/수정/임베딩 등)과 verified 상태를 dev에 자동 확정(`ofl_verify.py`, progress-008.md 참고). 2026-07-21 prod에도 `--target prod` + 이중 게이트(공식 확인 + dev verified 교차)로 Tier A OFL 128종 승격 적용 — fontagit.com 실서비스 라이선스 verified 128/pending 8.
...
- 홈 통합 비교 보드: 캔버스(대표 96px + 8칸 그리드)와 비교(셀렉트) 화면을 홈 하나로 통합, 무료 9종 전 칸 폰트 교체 + 프리셋, /playground는 302로 홈 이동(v0.9.0)
- 폰트 출처 정보 정확화: 폰트 상세에 실제 제작사(권리사)와 공식 다운로드 페이지를 표시하고, 출처를 등급(제작사 공식 / 공공기관 / 재배포 아카이브)으로 구분. 구글폰트-눈누 같은 아카이브가 제작사인 것처럼 보이던 문제 해소. Tier A 126종 prod 반영(v0.11.3), 나눔명조 제작사가 "네이버"로 정정됨

## 진행 기록

## 2026-07-31 - 플레이그라운드 캔버스 편집기 설계 듀얼 리뷰 반영 (#69)
- 상태: 완료 (설계 확정, 구현 미착수)
- 요약: 1차 설계 문서를 Codex와 Gemini 두 모델에 병렬로 리뷰시켜 지적 23건을 반영했다. 자유 크기 한 변 4096이 문서 자체의 픽셀 상한 1600만을 넘던 산술 모순(4096x4096=1677만), `useSearchParams`의 Suspense 경계 누락으로 인한 정적 빌드 실패 위험, 되돌리기 스냅샷 대상 누락, 캔버스 크기 변경 시 배율이 `fontSize`와 `scale` 양쪽에 곱해지는 이중 적용이 핵심 결함이었다. 폰트 메타데이터 계약-기본값-완료 조건 절을 새로 두어 구현자가 임의 판단할 여지를 없앴다.
- 커밋/PR: `920865f` docs: 듀얼 리뷰 지적 23건 설계 반영 (#69). PR 미생성
- 남은 일: (1)PR 생성 (2)구현 첫 단계 실험 5종 - Fabric 7.4.0의 텍스트 객체 클래스명, 한글 조합 입력, JSON 왕복, React 이중 초기화, PNG 출력 확인 (3)실험 결과에 따라 설계 재조정 후 구현 착수
- 관련 문서: `docs/superpowers/specs/2026-07-31-playground-canvas-editor-mvp-design.md`, 리뷰 판정 근거는 `docs/review/review-result-dual-20260731-141652.md`
- 상세: 없음

## 2026-07-28 - 눈누 출처 URL 오염 원인 규명 및 수집기 수정 (#148, PR #149)
- 상태: 부분 완료 (수집기 수정 머지 완료 / 저장된 데이터 정정은 #150으로 이관)
- 요약: "출처가 눈누 인스타그램으로 연결된다"는 제보(#148)를 조사해 1종이 아닌 172종 문제임을 확인했다. 수집기가 페이지 전체에서 `.com`/`instagram` 등이 포함된 첫 링크를 제작사로 확정해, 진짜 제작사 링크(`clova.ai`)는 목록에 없어 놓치고 푸터의 눈누 공식 SNS를 집었다. 링크 탐색을 본문 영역으로 한정하고 눈누 자체 도메인을 차단하며 TLD 화이트리스트를 제거했다. 본문 영역을 못 찾으면 값을 비우는 fail-closed 설계.
- 커밋/PR: `0dd2548` PR #149 머지 완료
- 남은 일: (1)#150 오염 전수 검증과 172종 정정 - 수정된 수집기로 재수집 필요 (2)938종 미검증 위험 (3)#141 크롤러 결함 (4)#142 체크포인트 결함
- 관련 문서: 없음
- 상세: progress-015.md

## 2026-07-28 - Tier A 제작사/다운로드 정보 prod 반영 (#96 #120, PR #135~#143)
- 상태: 부분 완료 (prod DB 반영 126종 완료, 웹 재배포는 미실행 — 다른 세션의 홈화면 개편 #144/#145가 함께 나가게 되어 사용자 확인 대기)
- 요약: 승인된 Tier A 감사 결과를 dev(128종)와 prod(126종)에 적용해 나눔명조 제작사가 구글폰트가 아닌 "네이버"로 정정됨을 양쪽에서 확인했다(#120 문제점 1번). 적용을 막던 결함 3종을 고쳤다 — SQL NULL과 JSON null을 다르게 본 오탐 conflict(291건 전량 차단), 실패 원인을 알 수 없던 예외 메시지, dev에만 있는 폰트 2종이 prod 빌드 전체를 중단시키던 문제. apply 전에 manifest와 DB를 필드 단위로 대조하는 preflight를 신설해 기본 게이트로 넣었다.
- 커밋/PR: PR #135 #136 #137 (승인 경로), #139 (마이그레이션 0025), #140 (preflight), #143 (제외 처리) 전부 머지
- 남은 일: (1)웹 재배포로 fontagit.com 반영 (2)크롤러 결함 5종 + 데이터 오염 의심 #141 (3)체크포인트 실패 기록 결함 #142 (4)눈누 22종 수집 (5)KOGL 31건 사람 승인 (6)pgTAP #134
- 관련 문서: docs/superpowers/specs/2026-07-27-license-audit-crawl-design.md
- 상세: progress-014.md
...
- 커밋/PR: squash `094a953`, PR #127 (https://github.com/jojo3420/fontagit/pull/127), 태그 v0.9.0
- 남은 일: (1)E2E 상세 404 근본 원인 조사 후 상세 스냅샷 기준선 재생성 (2)Codex Should-fix 3건(E2E 검증 강화, aria 이름) — docs/review/pr-review-127-20260727-152601.md (3)#69 캔버스 편집기(비워둔 /playground 사용)
- 관련 문서: docs/superpowers/specs/2026-07-27-compare-canvas-home-merge-design.md, docs/superpowers/plans/2026-07-27-home-compare-canvas.md
- 상세: progress-012.md
...
- 상태: 부분 완료 (조립 파이프라인 코드 완료, 실데이터 파일럿은 다음 세션 - dev 쓰기+눈누 크롤 하드 게이트)
- 요약: 눈누 tags/weights를 감사(audit) 파이프라인으로 dev fonts에 채우는 코드 완성. Docker 이미지+크롤 429/503 backoff(준비), assert_safe 게이트 metadata 교정(needs_review 대신 broken 비율 판정), approve_finding(검수 게이트 보존), build_manifest 입력 조립 헬퍼 3종, `font-audit-manifest build` CLI. 원래 plan 실행경로가 실코드와 어긋남을 dry-run(exit3)으로 실증해 REVISED plan으로 6단계 재설계.
- 커밋/PR: PR #99 `3b666d2`(준비 인프라+REVISED plan), PR #101 `35a5598`(Task1~4 조립 파이프라인) 모두 develop squash 머지. 두 PR 모두 codex 리뷰 Must-fix 반영(#99 Docker venv-Retry-After 등 4건, #101 조회 페이지네이션 등)
- 남은 일: 1)metadata approve CLI(거버넌스 결정: per-finding vs 파일럿 batch) 2)Task5 파일럿 6단계(prod기준선→bootstrap→감사→승인→build→apply, dev 쓰기) 3)supabase-dev MCP 연결 확인
- 관련 문서: handoff/2026-07-23-collections-phase0-pilot.md, plans/2026-07-22-collections-phase0-execution-REVISED.md
- 상세: 없음

## 2026-07-22 - 컬렉션 확장 0단계: 데이터 정비 설계 + Task1 구현 (PR #98)
- 상태: 부분 완료 (0단계 코드 Task1 완료 + develop 머지, Task2~3 실행은 준비 plan만 - 도커/manifest CLI/안전장치 코드 미착수) [2026-07-23 갱신: 준비 중 Docker 이미지-크롤 backoff는 PR #99로 구현 완료. manifest CLI 등 실제 실행은 execution-REVISED plan으로 이관됨 — 이 항목은 그 이전 시점 기록]
- 요약: /collections가 3개뿐인 병목이 "폰트 분류 데이터 결측"임을 dev 실측으로 규명(category_ko 고딕 96.6% 편중, tags 전부 빈값, weights 72% 결측). 해결은 신규 파이프라인이 아니라 기존 신 감사(audit) 경로(audit_noonnu→audit_runner→apply RPC)로 tags/weights를 채우는 것. 0단계 코드 갭(눈누 tags를 metadata evidence에 연결)을 Task1로 구현+테스트. 방향=역할재정의(/fonts 용도 vs /collections 라이선스+에디토리얼)+DB 규칙엔진+B→A 단계. Codex 리뷰가 빈태그 삭제 버그(빈 배열이 태그삭제 finding 생성)를 잡아 `and parsed.tags` 가드 추가.
- 커밋/PR: PR #98 squash 머지 `bac4896` (https://github.com/jojo3420/fontagit/pull/98, MERGED). Task2~3 준비 plan `eb69a3c` (브랜치 feature/collections-task23-prep)
- 남은 일: Task2~3 실행 준비(Docker 이미지, manifest 생성 CLI, 크롤링 P0 재시도/backoff) → 파일럿 50종 → 전체 1,110 dev apply → B단계 plan(DB 규칙엔진 + 자동 컬렉션)
- 관련 문서: specs/2026-07-22-collections-expansion-design.md, plans/2026-07-22-collections-phase0-data-enrichment.md, plans/2026-07-22-collections-phase0-task23-execution-prep.md, review/review-result-dual-20260722-153145.md
- 상세: 없음 (설계/plan 문서로 대체)

## 2026-07-22 - 글자 지원 검사를 폰트 상세로 이동 (#97) + v0.3.0 prod 배포
- 상태: 완료
- 요약: "글자 지원 검사"(특정 폰트에서 글자 지원 여부를 canvas로 검출)를 /playground 하단에서 폰트 상세(/fonts/[slug]) 견본 아래로 이동. 상세는 폰트가 고정이라 선택 드롭다운 제거하고 현재 폰트로 GlyphChecker 직접 렌더, 검사 가능한 무료 웹폰트(next/font/google 8종)에서만 노출하고 유료/로컬(pretendard)은 섹션 숨김. 판정식을 `isGlyphCheckSupported`로 추출해 페이지 게이트와 컴포넌트 내부 게이트를 정합(오용 방지). playground 전용이던 GlyphCheckerSection 제거-삭제. 검사기의 @font-face는 layout의 fontClassNames로 전역 로드돼 상세 페이지에서도 정상 동작(적대적 리뷰로 확인 — SpecimenBox 로딩에 비의존).
- 커밋/PR: main ff 승격 코드 4커밋 `2c66118`..`76bd582` (PR 없음, 세션 내 SDD 실행), prod 배포 태그 v0.3.0 (deploy.sh --new-tag, fontagit.com 스모크 200)
- 남은 일: 실제 글자 검출 동작은 canvas라 단위테스트 불가 → 브라우저/e2e 수동 검증 권장(선택). 이슈 #97 종료됨.
- 관련 문서: docs/superpowers/specs/2026-07-21-glyph-checker-move-design.md, docs/superpowers/plans/2026-07-21-glyph-checker-move.md
- 상세: 없음

## 2026-07-21 - Tier A OFL 128종 prod verified 승격 (#89 재편)
- 상태: 완료
- 요약: prod Tier A(Google Fonts) 폰트 라이선스 pending 136종 중 OFL 128종을 verified로 승격(실서비스 DB 반영, 잔여 pending 8). 원래 #89(URL 백필→needs_review) 설계를 Phase 0 실측으로 재편 — 새 CLI 대신 기존 `ofl_verify.py`에 `--target prod` + 이중 게이트(google/fonts 공식 확인 + dev verified 교차, 불일치는 pending 유지) 확장, `config.py`에 prod 쓰기 자격증명 경계(`prod_write_credentials`, env allowlist `SUPABASE_AUDIT_PROD_ALLOWLIST`) 신설. 검수 방식은 예외 기반 자동화로 전환(사용자 결정 — 기존 "자동 verified 금지" 게이트 갱신, prod 쓰기 직전 go/no-go 1회만).
- 커밋/PR: develop 로컬 머지 6커밋 `d215ba0`..`82fb6af` (PR 없음), 설계/계획 문서 `700af64`, `2e13537`
- 남은 일: #95 비-OFL 8종(Roboto Slab/Ubuntu/Google Sans/Material 계열) 개별 검수, #96 Tier A download_url 백필, #83 cmap 검증 CI, #28 컬렉션 시드
- 관련 문서: docs/superpowers/specs/2026-07-21-tier-a-url-backfill-design.md, docs/superpowers/plans/2026-07-21-tier-a-ofl-prod-promotion.md
- 상세: 없음
