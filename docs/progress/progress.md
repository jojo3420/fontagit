...
- 헤더 검색바 상시 노출: 아이콘을 눌러 펼치던 방식을 없애고 2단 헤더 아랫줄에 검색바를 항상 노출, 슬래시(/) 단축키로 어디서든 검색창 포커스(v0.8.0)

## 진행 기록

## 2026-07-27 - 헤더 검색바 상시 노출 + Next 한글 slug dev 패치 (#117)
- 상태: 완료 (v0.8.0, v0.8.1 prod 배포)
- 요약: 검색 아이콘 토글과 펼침 패널을 없애고 2단 헤더 아랫줄에 검색바를 상시 노출(슬래시 단축키 추가, 중복되는 홈 히어로 검색창 제거), 헤더 z-index 상향과 `--header-h` 토큰으로 캔버스 바 겹침을 해소했다. 별건으로 dev 서버에서 한글 slug 폰트 상세가 500이던 Next 버그를 `pnpm patch`로 고정 — 업스트림 PR 미병합이라 버전 업그레이드로는 해결 불가하고, `generateStaticParams` 인코딩 우회안은 prod 404를 유발해 폐기했다.
- 커밋/PR: `27e45a0` 헤더 검색바, `8484237` Next 패치, PR #121-#123, 릴리스 PR #122(v0.8.0)-#124(v0.8.1)
- 남은 일: 홈 모바일 375px 가로 넘침(develop에서도 재현되는 기존 버그, 이슈 미등록), 업스트림 vercel/next.js#92194 병합 시 `patches/next@16.2.10.patch` 제거
- 관련 문서: docs/mockups/2026-07-27-header-search-always-on/layout-options.html
- 상세: 없음
