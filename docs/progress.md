...
- 남은 일: (1) 슬라이스3 클릭 집계(마이그레이션 0007). (2) 검색 이연분 — 오류 상태 UI 구분(0건과 장애 구분), DB 통합 테스트(테스트 인프라 부재), URL q 변경 동기화, prod 배포 시 0006 적용. (3) 슬라이스 0.5 이연분(Codex Medium 4건)은 별도 처리.
