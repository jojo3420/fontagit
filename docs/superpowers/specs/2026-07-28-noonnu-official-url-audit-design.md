...
- 본문에 제작사 링크가 아예 없는 폰트의 `official_url`을 NULL로 둘지 (⚠️ 2026-07-29 실측: `fonts.official_url`은 NOT NULL 컬럼이라 이 선택지는 스키마 제약 완화(0027)까지 포함하는 결정이다. `license_source_url`은 nullable이라 해당 없음)
