...
> 작성일: 2026-07-18
> 상태: 사용자 설계 승인, 자체 적대적 리뷰·Claude 외부 리뷰·한글 판별 요구사항 보강
...
2. 실제 한글·라틴 지원 범위와 폰트명·분류·굵기·스타일 등 나머지 정보를 보정한다.
...
- Tier B 눈누 seed 1,157건은 `subsets` 필드 자체가 없고 importer도 이 값을 쓰지 않아 DB 기본값인 빈 배열로 남는다.
...
5. Google Fonts가 제공하는 `subsets`를 Tier B에도 있다고 가정했지만 눈누 seed·import·enrich 어디에서도 문자 지원 범위를 생성하지 않았다. `source_tier='B'`는 출처 구분일 뿐 한글 글리프 지원 증거가 아니다.
...
| 한글 판별 | 실제 폰트 파일의 Unicode cmap 우선. 파일 근거가 없으면 추정하지 않고 재확인 |
...
| `subsets` | 실제 파일 근거의 `korean` / `latin` 지원 목록 |
| `script_status` | `pending` / `verified` / `needs_review` |
| `script_evidence_id` | 문자 지원 판정 내부 근거 스냅샷 ID |
| `script_checked_at` | 문자 지원 마지막 확인일 |
...
- `script_status='verified'`이면 `subsets`가 비어 있지 않고 `script_checked_at`, `script_evidence_id`가 있어야 한다.
- `subsets=[]`는 영문 폰트가 아니라 `미확인`이다. `source_tier`, 한글 폰트명, 눈누 등록 여부만으로 `korean`을 넣지 않는다.
...
`download_evidence_id`, `license_evidence_id`, `script_evidence_id`는 `font_source_snapshots.id`를 `on delete restrict`로 참조한다. audit apply RPC는 evidence의 font ID, provider, source kind, document kind가 갱신 필드와 일치하는지 다시 검사한다.
...
가능하면 공식 배포 폰트 파일을 `fontTools`로 읽어 굵기·스타일과 Unicode `cmap`을 확인한다. WOFF2는 Brotli 지원을 함께 설치해 같은 방식으로 읽는다.

- KS X 1001의 한글 공통 완성형 2,350자가 실제 cmap에 모두 있으면 `korean` 지원으로 기록한다. 일부 한글만 있으면 확인된 다른 문자 지원은 보존하되 `korean`으로 자동 확정하지 않고 `needs_review`로 보낸다.
- 기본 라틴 대·소문자 A~Z·a~z가 모두 있으면 `latin` 지원으로 기록한다.
- 실제 파일의 family 이름이 대상 폰트와 일치하고 cmap을 정상 판독했을 때만 `script_status='verified'`로 자동 확정한다.
- 같은 family·굵기·스타일에 여러 분할 파일이 있으면 cmap 합집합으로 판정한다. 서로 다른 face의 판정이 충돌하면 자동 확정하지 않는다.
- CSS `unicode-range`, 요청 글자 파라미터, 불완전한 공통 한글 범위가 발견되면 미리보기용 부분 파일로 간주한다. 모든 분할 파일의 합집합을 확보하지 못한 부분 파일은 자동 판정 근거로 쓰지 않는다.
- CSS `unicode-range`, 눈누 페이지, 한글 폰트명은 파일을 찾기 위한 후보 근거일 뿐이다. 파일을 읽지 못하거나 family가 충돌하면 `needs_review`다.
- 실제 폰트 바이너리는 저장하지 않고 파일 해시, cmap 코드포인트 개수, 판정 범위, 파서 버전만 metadata snapshot에 저장한다.

페이지 설명과 파일 정보가 충돌하면 파일을 자동 변경 근거로 쓰지 않고 검수 대상으로 보낸다.
...
견본 문구는 `script_status='verified'`와 `subsets`로 고른다. `korean` 지원이면 한글 견본, 라틴만 지원하면 영문 견본을 사용한다. 미확인 상태에서는 Tier B라는 이유로 한글이라 단정하지 않고 한글·영문 혼합 확인 문구를 사용한다.
...
- 문자 판별: 분할 cmap 합집합의 한글+라틴, 라틴 전용, 부분 한글의 3개 핵심 사례
- 일반 메타데이터: 공식 파일의 굵기·이탤릭 확인, 파일명·family 충돌의 2개 핵심 사례
...
- 모든 폰트의 문자 지원이 verified 또는 needs_review이며 빈 `subsets`를 영문으로 오판하지 않는다.
...
- `source_tier='B'`만으로 한글 폰트로 판정하는 코드가 없다.
