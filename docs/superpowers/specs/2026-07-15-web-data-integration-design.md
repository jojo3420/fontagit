...
- 마이그레이션 `0006`: `pg_trgm` 확장 + `aliases.alias_norm` GIN trgm 인덱스 + `search_fonts(q text)` RPC(정확 별칭 > 부분일치 > trgm 유사도 점수화). anon에 execute 권한. (`0005`는 Tier A stale 폰트 동기화가 사용)
...
- 마이그레이션 `0007`: `font_clicks`(익명, IP-식별자 컬럼 없음, `font_id` FK, `clicked_at` timestamptz default now, 인덱스) + `font_click_daily`(롤업). **anon은 원본 테이블 직접 접근 불가**, `record_click`/`get_top_fonts` RPC로만.
...
### 슬라이스 4 — 등록폼 제출 (F-14)
- 마이그레이션 `0008`: `submissions`(name, foundry, category, official_url, license_choice, status='pending', created_at). anon 직접 insert 차단, `submit_font` RPC로만(status/created_at DB 강제).
