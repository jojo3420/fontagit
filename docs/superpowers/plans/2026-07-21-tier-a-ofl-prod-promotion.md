...
- [ ] **Step 1: 실패하는 테스트 작성** — `tests/test_ofl_verify.py` 끝에 추가 (import에 `fetch_dev_verified_keys` 추가):
...
- [ ] **Step 4: 전체 ofl_verify 테스트 green 확인**
...
- [ ] **Step 1: main() 확장** — 자격증명 로드 블록(`dev_url, dev_key = ...`부터 `headers = {...}`까지)을 교체:
...
- [ ] **Step 2: argparse 배선** — `--report-path`의 default를 `None`으로 바꾸고 `--target` 추가:
...
- [ ] **Step 3: env allowlist 추가** — 리포지토리 루트에서 (값을 화면에 출력하지 않고 SUPABASE_PROD_URL 값을 복사):
...
- [ ] **Step 4: 회귀 확인 (전체 스위트 + dev dry-run 무변경 확인)**
...
- [ ] **Step 5: Commit** (.env는 gitignore 대상이라 커밋 안 됨 — 코드만)
...
- [ ] **Step 3: go/no-go 승인 요청 (게이트 — 승인 없이 Task 5 착수 금지)**
...
- [ ] **Step 2: 쓰기→읽기 재검증** — 리포지토리 루트에서 prod 재조회(읽기 전용):
