...
- 목록의 400과 상세 견본의 700 중 실제 `availableWeights`에 존재하는 굵기만 CSS2 `wght` 축으로 요청한다.
...
- 2026-07-18 운영 `https://fontagit.com/` 응답에는 CSP 헤더가 없음을 확인했다. 추후 CSP를 추가하면 `style-src https://fonts.googleapis.com`, `font-src https://fonts.gstatic.com` 허용이 필요하다.
- Tier A의 `name_en`은 Google Webfonts API의 `family` 원문이라는 파이프라인 불변식을 사용한다.
...
describe("resolveFontPreview", () => {
  it("미매핑 Tier A 폰트를 Google CSS2 URL과 실제 family로 연결한다", () => {
    expect(resolveFontPreview({
      ...
      stylesheetUrl: "https://fonts.googleapis.com/css2?family=Orbitron%3Awght%40400%3B700&display=swap",
    });
  });
...
type PreviewFont = Pick<Font, "fontKey" | "nameEn" | "sourceTier" | "availableWeights">;
...
export function resolveFontPreview(font: PreviewFont): FontPreviewResolution {
  ...
  const familyQuery = previewWeights.length > 0
...
export function LazyFontPreview({
  ...
  font: Pick<Font, "fontKey" | "nameEn" | "sourceTier" | "availableWeights">;
  className?: string;
...
expect(Array.from(document.fonts).some(
  (face) => face.family === "Orbitron" && face.weight === "400" && face.status === "loaded"
)).toBe(true);
expect(Array.from(document.fonts).some(
  (face) => face.family === "Orbitron" && face.weight === "700" && face.status === "loaded"
)).toBe(true);
```
