...
- 눈누 페이지 문구와 일반 검색 결과만으로 다운로드·라이선스를 verified로 만들지 않는다. 같은 상세 페이지의 실제 @font-face 파일은 문자 지원 판정에만 제한적으로 사용할 수 있다.
...
- source_tier는 출처 구분일 뿐 언어 판별 근거로 사용하지 않는다.
- subsets 빈 배열은 영문이 아니라 미확인이다. 실제 폰트 cmap 근거로 korean/latin을 채운다.
...
| apps/pipeline/src/fontagit_pipeline/audit_metadata.py | fontTools cmap 기반 문자 지원과 굵기·스타일 후보 생성 |
...
착수 전 실제 스키마에서 `subsets`가 `text[] not null default '{}'`인지, 0016의 allow_embedding/allow_redistribute/allow_modify CHECK가 `allowed|conditional|denied`를 강제하는지 검사한다. 하나라도 다르면 migration을 중단하고 0017에 명시적 보정 DDL을 먼저 추가한다.
...
7. 허용 필드만 한 트랜잭션에서 update하고 updated_at을 갱신
...
정방향 manifest는 stale write 차단을 위해 expected_updated_at을 검사한다. 역방향 manifest는 정방향 적용 전 함께 만들되 rollback_mode로 표시하고, 적용 시 updated_at 대신 현재 감사 대상 필드가 정방향 after와 정확히 같은지 검사한다. 따라서 정방향 적용으로 updated_at이 바뀌어도 되돌릴 수 있고, 감사 대상 필드가 이후 수정됐으면 역방향 전체를 거부한다.

font_audit_manifest_test.sql은 다음 3개 트랜잭션 케이스만 둔다.

- 전부 일치: 2건 모두 적용
- 한 행 before 불일치: 예외 후 0건 변경
- 정방향 적용으로 updated_at이 바뀐 뒤 역방향 적용: 대상 필드 byte-for-byte 동일
...
- Modify: apps/web/lib/specimen.ts
- Modify: apps/web/lib/specimen.test.ts
...
- Font.scriptStatus: pending | verified | needs_review
- resolveSpecimenLanguage(font) -> korean | english | mixed
...
specimen.test.ts에는 출처 tier가 아닌 검증 상태와 subsets만 판별하는 3개 사례를 둔다.

~~~typescript
it("verified korean subset은 한글 견본을 선택한다", () => {
...
it("verified latin-only subset은 영문 견본을 선택한다", () => {
  expect(resolveSpecimenLanguage({
    subsets: ["latin"],
    scriptStatus: "verified",
    sourceTier: "B",
...
it("빈 subsets는 Tier B여도 mixed 확인 문구를 선택한다", () => {
  expect(resolveSpecimenLanguage({
    subsets: [],
...
~~~bash
pnpm --dir apps/web test --run __tests__/mappers.test.ts lib/specimen.test.ts
~~~

Expected: licenseAudit/downloadUrl/scriptStatus와 resolveSpecimenLanguage가 없어 실패.
...
licenseAudit.sourceMode는 라이선스 표시만 위한 값이다. 새 라이선스 상태가 pending이고 신규 라이선스 근거가 전혀 없을 때만 legacy로 읽는다. 다운로드와 문자 지원은 각각 downloadStatus와 scriptStatus로 독립 판정한다. downloadStatus가 broken이면 “다운로드 링크 확인 불가”, licenseStatus가 needs_review이면 “라이선스 재확인 필요”로 서로 다르게 표시한다. downloadUrl은 download_status = verified일 때만 설정한다.

subsets는 빈 배열을 latin으로 해석하지 않는다. scriptStatus가 verified이고 subsets에 korean이 있으면 한글, verified이고 latin만 있으면 영문이다. 그 외에는 한글·영문 혼합 확인 문구를 사용한다. 기존 sourceTier === "B" 한글 판별은 제거한다.
...
git add apps/web/lib/db/types.ts apps/web/lib/db/mappers.ts \
  ...
  apps/web/lib/specimen.ts apps/web/lib/specimen.test.ts \
...
import는 사람이 dev service key로 실행한다. 해시·스키마·run_id 중복을 검사하고 관찰 snapshot/finding만 저장한다. 404·410의 24시간 판정은 `audit_http.classify_download`를 호출하며 같은 규칙을 다시 구현하지 않는다. prod를 수정하지 않는다.
...
### Task 11: 한글·라틴 글리프 판별과 2단계 메타데이터 감사
...
- Produces: classify_scripts(codepoints) -> ScriptCoverage
- Produces: classify_face_scripts(cmaps) -> ScriptCoverage
- Produces: compare_metadata(target, official_snapshot, file_metadata) -> list[FindingDraft]
- CLI: font-audit-run --stage metadata

- [ ] **Step 1: 글리프 판별 3개와 일반 메타데이터 2개 테스트 작성**

~~~python
def test_split_files_are_unioned_before_korean_classification():
    shard_a, shard_b = split_in_half(KS_X_1001_HANGUL)
...
def test_basic_latin_only_cmap_is_not_korean():
    coverage = classify_scripts(set(BASIC_LATIN))
    assert coverage.subsets == ["latin"]
    assert coverage.status == "verified"


def test_partial_hangul_requires_review():
...
def test_page_file_family_conflict_requires_review():
...
~~~bash
cd apps/pipeline
uv add 'fonttools[woff]'
~~~

공식·공공기관 파일을 우선하고, 없으면 해당 눈누 상세 페이지의 @font-face 파일을 문자 지원 판별 근거로만 사용한다. 폰트 다운로드는 Task 4의 DNS·리다이렉트 검사를 재사용하되 압축 파일 최대 32 MiB, WOFF/WOFF2의 선언된 해제 크기 최대 128 MiB, 파싱 시간 제한을 별도로 둔다. 파일은 임시 파일로 읽고 DB에 바이너리를 저장하지 않는다. TTFont.getBestCmap(), name table, OS/2 usWeightClass, head/macStyle, post/italicAngle을 구조화 값과 파일 SHA-256으로 저장한다. WOFF2 판독을 위해 fonttools의 woff extra로 Brotli 의존성을 잠근다.

classify_scripts는 아래 범위를 사용한다.
...
BASIC_LATIN = set(range(0x41, 0x5B)) | set(range(0x61, 0x7B))
~~~

KS X 1001 공통 한글 2,350자가 모두 있으면 subsets에 korean을 넣는다. 기본 라틴 52자가 모두 있으면 latin을 넣는다. 둘 다 있으면 두 값을 모두 저장한다. 한글이 일부만 있으면 확인된 latin은 보존하되 status는 needs_review다. 빈 cmap 또는 어느 범위도 충족하지 못하면 subsets를 비워 두고 needs_review다. 오판 분석을 위해 전체 한글 글리프 수와 공통 한글 충족 수를 함께 저장한다. 2,350자 기준은 파일럿에서 partial 비율을 측정하되 자동으로 낮추지 않고, 기준 변경은 별도 승인 규칙과 회귀 픽스처를 요구한다.

파일 name table family 또는 같은 @font-face의 font-family가 대상 폰트명과 일치하고 cmap을 정상 판독했을 때만 script_status를 verified로 만든다. 같은 family·굵기·스타일의 분할 파일은 개별 분류 전에 cmap 합집합을 사용하고, 서로 다른 face의 문자 지원 판정이 충돌하면 needs_review다. CSS unicode-range, 요청 글자 파라미터 또는 불완전한 공통 한글 범위가 있는데 모든 분할 파일을 확보하지 못하면 미리보기용 부분 파일로 보고 자동 확정하지 않는다. 페이지와 파일의 폰트명이 다르거나 파일에 여러 family가 있으면 모든 제안을 needs_review로 만든다. 이름이 일치할 때만 weights와 variants를 자동 적용 후보로 만든다. category와 tags는 공식 페이지 값이 있어도 검수 후보로만 둔다.

- [ ] **Step 4: runner/CLI 연결**

metadata stage는 legal stage의 official/public 파일을 먼저 사용한다. 없으면 같은 눈누 font snapshot에서 추출한 @font-face 파일만 허용한다. CSS unicode-range, source_tier, 한글 폰트명만으로 verified를 만들지 않는다. 파일 URL이 discovery 도메인에만 있으면 다운로드하지 않고 needs_review로 둔다.
...
Expected: 신규 5개와 전체 pipeline 테스트 PASS. 한 메서드당 사례는 최대 3개다.
...
git commit -m "feat: detect font script coverage from cmap"
~~~
...
uv run python -m fontagit_pipeline font-audit-run \
  ...
  --out output/audit/pilot-legal
uv run python -m fontagit_pipeline font-audit-run \
  --stage metadata --limit 50 \
  --require-slug 흰꼬리수리 \
  --require-slug 횡성한우체 \
  --out output/audit/pilot-metadata
~~~
...
- metadata 50종의 partial 한글·부분 파일·분할 파일 누락 수가 각각 보고됨
- needs_review 비율이 10% 이하면 다음 단계
- legal 또는 metadata needs_review가 10%를 초과하면 지배적인 실패 원인을 보강하고 Step 3을 재실행. 한글 기준값을 통과 목적으로 임의 완화하지 않음
...
- 모든 대상의 script_status가 verified 또는 needs_review이며 pending은 0
- verified이면 subsets에 korean 또는 latin이 하나 이상 있고 script evidence와 확인일이 있음
- 파일을 읽지 못했거나 family가 충돌하면 빈 subsets + needs_review이며 영문으로 분류하지 않음
- 미리보기용 부분 파일 의심 수, 모든 분할 파일 합집합 확보 수, partial 한글 비율을 별도 보고
...
- 문자 지원 verified/needs_review와 korean/latin/빈 subsets 수
- 미리보기용 부분 파일과 partial 한글 수
...
적용 직전 published 대상의 download_status, license_status, script_status별 pending 수를 한 쿼리에서 검사한다. 어느 축이든 pending이 1건 이상이면 0019를 실행하지 않는다.

~~~sql
select
  count(*) filter (where download_status = 'pending') as download_pending,
  count(*) filter (where license_status = 'pending') as license_pending,
  count(*) filter (where script_status = 'pending') as script_pending
from fontagit.fonts
...
alter table fontagit.fonts
  ...
  add constraint fonts_verified_script_complete check (
    script_status <> 'verified'
    or (
      cardinality(subsets) > 0
      and script_checked_at is not null
      and script_evidence_id is not null
    )
  );
~~~
...
- verified korean 폰트: 한글 견본 표시
- verified latin-only 폰트: 영문 견본 표시
- 문자 지원 미확인 폰트: 한글·영문 혼합 확인 문구 표시
...
where status='published' group by 1 order by 1;
select script_status, count(*) from fontagit.fonts
where status='published' group by 1 order by 1;
select coalesce(array_to_string(subsets, ','), ''), count(*)
from fontagit.fonts where status='published' group by 1 order by 1;
"
...
- 기준선 전체의 문자 지원 상태가 verified 또는 needs_review이며 pending이 없다.
- 빈 subsets는 영문으로 취급하지 않고, source_tier='B'만으로 한글을 판정하지 않는다.
